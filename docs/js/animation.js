...
import { keyboardSelection, setInputDisabled, setKeyboardSelection, } from "./keyboard.js";
...
function animationEnd(reverse = false) {
    Array.from(el.links.children).forEach((el, i) => (el.style.transitionDelay = 0.05 * i + "s"));
    if (reverse) {
        [el.title, el.pathBack].forEach((e) => {
...
export async function goBack() {
    ...
    const animDone = sleep(200);
    // wait for page to change
    await router.goto(("/" + router.path.split("/").slice(0, -1).join("/")).replace("//", "/"));
    let titleOffset = el.title.getBoundingClientRect().width;
    [el.title, el.pathBack].forEach((e) => {
